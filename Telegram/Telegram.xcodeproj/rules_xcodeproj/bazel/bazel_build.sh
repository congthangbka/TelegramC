...
readonly bazel_cmd=(
  env
  'PATH=/bin:/usr/bin'
  "/Users/ThangNC/Git/SourceCode/Telegram-iOS/build-input/bazel-6.3.2-darwin-arm64"
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/Telegram/Telegram_xcodeproj/Telegram_xcodeproj_target_ids")
